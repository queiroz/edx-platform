...
def delete_course_and_groups(admin_user, course_id, commit=False):
    """
    ...
    if delete_course(module_store, content_store, loc, commit):
        ...
        if commit:
            try:
                ...
                staff_role.remove_users(admin_user, *staff_role.users_with_role())
                instructor_role = CourseInstructorRole(loc)
                instructor_role.remove_users(admin_user, *instructor_role.users_with_role())
